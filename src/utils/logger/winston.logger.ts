...
export class WinstonLogger implements LoggerService {
    private colorizeOutput: boolean = [
        AllowedNodeEnvs.production,
        AllowedNodeEnvs.uat,
        AllowedNodeEnvs.staging,
    ].includes(process.env.NODE_ENV as AllowedNodeEnvs)
        ? true
        : true;
    ...
    customFormatting = format((info: TransformableInfo, options) => {
        ...
        const timestampDiff = WinstonLogger.updateAndGetTimestampDiff(this.isTimestampEnabled, options.colorizeOutput);
    ...
    constructor(@Optional() protected context?: string, @Optional() private isTimestampEnabled = false) {}
    ...
    printMessage(level: keyof CustomLoggingLevels<AllowedLogLevels>['levels'], message: string, ...meta: any[]) {
    ...
    getPadding(currentInfo: TransformableInfo) {
        const maxLevelLengthPlusOne =
            Math.max(...Object.keys(WinstonLogger.customLoggingLevels.levels).map((level) => level.length)) + 1;
    ...
    colorizeByCustomLevel(level: string, toFormat: string): string {
        const colorFunc = WinstonLogger.customLoggingLevels.colors[level] ?? clc.white;
        return colorFunc(toFormat);
    }

    private static updateAndGetTimestampDiff(isTimeDiffEnabled?: boolean, colorize?: boolean): string {
        const includeTimestamp = WinstonLogger.lastTimestamp && isTimeDiffEnabled;
        const formattedTimestamp = ` +${Date.now() - WinstonLogger.lastTimestamp}ms`;
        const result = includeTimestamp ? (colorize ? clc.yellow(formattedTimestamp) : formattedTimestamp) : '';
